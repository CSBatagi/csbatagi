/*
*	CSStatsX SQL						v. 0.7.4+2a
*	by serfreeman1337		https://github.com/serfreeman1337
*   modified for CS Batagi by bobinaj
*/
...
#define VERSION "0.7.4+2a"
#define AUTHOR "serfreeman1337 + CSB"

#define LASTUPDATE "15, May(05), 2021"
...
{
	static damage_take, net_damage, user_hp; 
	//user_hp = get_user_health(player);
	user_hp = pev(player, pev_health);
	damage_take = read_data(2);
	if(user_hp < 0){
		net_damage = damage_take + user_hp
	}
	else {
		net_damage = damage_take
	}
	...
	if(player_data[killer][PLAYER_LOADSTATE] == LOAD_OK && player_data[victim][PLAYER_LOADSTATE] == LOAD_OK) // ????? ????????????? ?????? ??? ??????? ?????????? ?? ??
	{
		switch(get_pcvar_num(cvar[CVAR_SKILLFORMULA])) // ?????? ??????
		{
			...
			case 0: // The ELO Method (http://fastcup.net/rating.html)
			{
				...
				new Float:killer_koeff = 0.75
				new Float:victim_koeff = 0.75
